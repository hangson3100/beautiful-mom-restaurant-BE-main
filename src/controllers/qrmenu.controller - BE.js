...
exports.getQRMenuOrderStatus = async (req, res) => {
  try {
    const { orderId } = req.params;
    if (!orderId) {
      return res.status(400).json({ success: false, message: 'Không tìm thấy mã đơn hàng!' });
    }

    const conn = await getMySqlPromiseConnection();

    const [orderRows] = await conn.query(`
      SELECT id, status, payment_status, token_no, customer_type, date
      FROM orders
      WHERE id = ?
      LIMIT 1
    `, [orderId]);
    ...
    const [itemRows] = await conn.query(`
      ...
      LEFT JOIN menu_item_variants miv ON miv.id = oi.variant_id AND miv.item_id = oi.item_id
      WHERE oi.order_id = ?
      ORDER BY oi.id DESC
    `, [order.id]);

    const hasPrepared = itemRows.some((item) => item.status === 'preparing');
    const hasCompleted = itemRows.some((item) => item.status === 'completed' || item.status === 'delivered');
    ...
    return res.status(200).json({
      ...
      items: itemRows.map((item) => ({
        ...
        status: item.status,
        displayStatus: itemStatusText[item.status] || 'Chờ xác nhận',
        notes: item.notes,
    ...
    console.error(error);
    return res.status(500).json({ success: false, message: 'Không thể lấy trạng thái đơn hàng!' });
  }
};
